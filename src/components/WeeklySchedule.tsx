...
interface WeeklyScheduleProps {
  ...
  practicas: Practica[];
  onRemoveFromSchedule: (horarioId: string) => void;
}

export const WeeklySchedule = ({ horarios, practicas, onRemoveFromSchedule }: WeeklyScheduleProps) => {
            ...
            {franjasHorarias.map(franja => (
              <React.Fragment key={franja}>
                ...
                {diasSemana.map(dia => {
                  ...
                  const practica = horario ? getPracticaById(horario.practicaId) : null;
                  
                  return (
                    <Card key={`${dia}-${franja}`} className="p-2 min-h-[80px] relative">
                      {practica ? (
                        <div className={cn(
                          'h-full rounded p-2 text-xs relative group',
                          `bg-${getModuloColor(practica.modulo)}-light border border-${getModuloColor(practica.modulo)}/20`
                        )}>
                          <Button
                            size="sm"
                            variant="ghost"
                            ...
                            onClick={() => onRemoveFromSchedule(horario.id)}
                          >
                            <X className="h-3 w-3" />
                          </Button>
